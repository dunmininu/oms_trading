...
from .models import Execution, Order, Position
...
def order_post_save_handler(sender, instance, created, **kwargs):
    ...
    try:
        if created:
            ...
            logger.info(
                f"Order created: {instance.client_order_id} - {instance.side} {instance.quantity} {instance.instrument.symbol}"
            )
        else:
            # Log order updates
            AuditLog.objects.create(
                ...
                metadata={
                    ...
                    "average_price": (
                        str(instance.average_price) if instance.average_price else None
                    ),
                },
            )
            logger.info(
                f"Order updated: {instance.client_order_id} - State: {instance.state}"
            )

    except Exception as e:
        logger.error(
            f"Error handling order post_save for {instance.client_order_id}: {e}"
        )
...
def order_pre_save_handler(sender, instance, **kwargs):
    ...
    try:
        if instance.pk:  # Only for updates, not new instances
            ...
            if old_instance.state not in [
                "FILLED",
                "PARTIALLY_FILLED",
            ] and instance.state in ["FILLED", "PARTIALLY_FILLED"]:
            ...
            if old_instance.state not in [
                ...
                "REJECTED",
                "EXPIRED",
            ] and instance.state in ["CANCELLED", "REJECTED", "EXPIRED"]:
        ...
        logger.error(
            f"Error handling order pre_save for {instance.client_order_id}: {e}"
        )
...
def execution_post_save_handler(sender, instance, created, **kwargs):
    ...
    try:
        if created:
            ...
            logger.info(
                f"Execution created: {instance.execution_id} - {instance.quantity} @ {instance.price}"
            )

    except Exception as e:
        logger.error(
            f"Error handling execution post_save for {instance.execution_id}: {e}"
        )
...
def position_post_save_handler(sender, instance, created, **kwargs):
    ...
    try:
        if created:
            logger.info(
                f"Position created: {instance.instrument.symbol} - {instance.quantity}"
            )
        else:
            logger.info(
                f"Position updated: {instance.instrument.symbol} - {instance.quantity} @ {instance.average_cost}"
            )

    except Exception as e:
        logger.error(
            f"Error handling position post_save for {instance.instrument.symbol}: {e}"
        )
        ...
        with transaction.atomic():
            ...
            if order.side in ["BUY", "BUY_TO_COVER"]:
                ...
                if new_quantity != 0:
                    ...
                    total_cost = (
                        position.quantity * (position.average_cost or Decimal("0.0000"))
            ...
            if order.average_price:
                ...
                total_value = (
                    order.average_price * (order.filled_quantity - execution.quantity)
            ...
            logger.info(
                f"Position updated for {instrument.symbol}: {new_quantity} @ {new_average_cost}"
            )

    except Exception as e:
        logger.error(
            f"Error updating position from execution {execution.execution_id}: {e}"
        )
...
def position_pnl_snapshot_handler(sender, instance, **kwargs):
    ...
    try:
        ...
        logger.error(
            f"Error handling position P&L snapshot for {instance.instrument.symbol}: {e}"
        )
...
def order_state_machine_handler(sender, instance, **kwargs):
    ...
    try:
        ...
        logger.error(
            f"Error handling order state machine for {instance.client_order_id}: {e}"
        )
