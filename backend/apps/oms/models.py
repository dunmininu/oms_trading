...
class Instrument(BaseModel):
    ...
    instrument_type = models.CharField(
        max_length=20, choices=INSTRUMENT_TYPE_CHOICES, db_index=True
    )
    ...
    option_type = models.CharField(
        max_length=4, choices=[("CALL", "Call"), ("PUT", "Put")], blank=True
    )

    # Trading Information
    min_tick_size = models.DecimalField(
        max_digits=10, decimal_places=6, default=Decimal("0.01")
    )
    min_order_size = models.DecimalField(
        max_digits=15, decimal_places=4, default=Decimal("1.0000")
    )
...
class Order(BaseModel):
    ...
    instrument = models.ForeignKey(
        Instrument, on_delete=models.CASCADE, related_name="orders", db_index=True
    )
    ...
    order_type = models.CharField(
        max_length=20, choices=ORDER_TYPE_CHOICES, db_index=True
    )
    ...
    time_in_force = models.CharField(
        max_length=20, choices=TIME_IN_FORCE_CHOICES, default="DAY"
    )
    state = models.CharField(
        max_length=20, choices=STATE_CHOICES, default="NEW", db_index=True
    )
    ...
    def clean(self):
        ...
        if self.order_type == "TRAILING_STOP" and not self.trailing_percent:
            raise ValidationError(
                "Trailing percent is required for trailing stop orders"
            )
...
class Position(BaseModel):
    ...
    instrument = models.ForeignKey(
        Instrument, on_delete=models.CASCADE, related_name="positions", db_index=True
    )
    ...
    def __str__(self):
        return (
            f"{self.instrument.symbol}: {self.quantity} @ {self.average_cost or 'N/A'}"
        )
    ...
    def update_market_value(self, market_price):
        ...
        if self.quantity != 0 and self.average_cost:
            ...
            if self.is_long:
                ...
                self.unrealized_pnl = (self.average_cost - market_price) * abs(
                    self.quantity
                )
        else:
...
class PnLSnapshot(BaseModel):
    ...
    # P&L Summary
    total_unrealized_pnl = models.DecimalField(
        max_digits=15, decimal_places=2, default=Decimal("0.00")
    )
    ...
    def net_pnl(self):
        """Calculate net P&L."""
        return (
            self.total_unrealized_pnl + self.total_realized_pnl - self.total_commission
        )
