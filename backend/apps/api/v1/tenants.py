...
class TenantRepository(DjangoRepository[Tenant]):
    ...
    def get_by_id(self, id: str, tenant_id: str | None = None) -> Tenant:
...
def list_tenants(request: HttpRequest, filters: Query[BaseFilterSchema] = Query(...)):
    """List all tenants with pagination and filtering."""
    try:
        result = tenant_service.repository.list(
            page=filters.page, page_size=filters.page_size, ordering=filters.ordering
        )

        return {
            "pagination": result["pagination"],
            "data": [
                TenantResponseSchema.from_orm(tenant) for tenant in result["data"]
            ],
        }
...
def list_tenant_users(
    request: HttpRequest, tenant_id: str, filters: Query[BaseFilterSchema] = Query(...)
):
...
def add_tenant_user(request: HttpRequest, tenant_id: str, data: TenantUserCreateSchema):
    """Add user to tenant."""
    try:
...
def create_tenant_invitation(
    request: HttpRequest, tenant_id: str, data: TenantInvitationCreateSchema
):
