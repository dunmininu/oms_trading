...
from apps.oms.schemas import (
    ...
    OrderUpdateSchema,
    PnLFilterSchema,
    PnLListResponseSchema,
    PnLSnapshotResponseSchema,
    PositionFilterSchema,
    PositionListResponseSchema,
    PositionResponseSchema,
)
from apps.oms.services import (
    ExecutionService,
    OrderService,
    PnLService,
    PositionService,
...
def list_orders(
    ...
    orders, total_count, has_next = service.list_orders(
        filters=query.dict(exclude_none=True), limit=query.limit, offset=query.offset
    )

    return OrderListResponseSchema(
        orders=orders, total_count=total_count, has_next=has_next
    )
...
@router.post(
    "/orders/bulk", response=BulkOrderResponseSchema, summary="Create Bulk Orders"
)
@api_controller
def create_bulk_orders(
    ...
    result = service.create_bulk_orders(
        orders=payload.orders, idempotency_key=idempotency_key
    )
...
@api_controller
def get_order(
    request: HttpRequest, order_id: str, tenant: Any = None, user: Any = None
):
    """
...
def update_order(
    ...
    order = service.update_order(
        order_id=order_id, updates=payload.dict(exclude_none=True), etag=etag
    )
...
def cancel_bulk_orders(
    ...
    result = service.cancel_bulk_orders(
        order_ids=payload.order_ids, idempotency_key=idempotency_key
    )
...
@router.get(
    "/executions", response=ExecutionListResponseSchema, summary="List Executions"
)
@api_controller
def list_executions(
    request: HttpRequest,
    ...
    return ExecutionListResponseSchema(
        executions=executions, total_count=total_count, has_next=has_next
    )
...
@api_controller
def get_execution(
    request: HttpRequest, execution_id: str, tenant: Any = None, user: Any = None
):
    """
...
def list_positions(
    ...
    return PositionListResponseSchema(
        positions=positions, total_count=total_count, has_next=has_next
    )


@router.get(
    "/positions/{position_id}", response=PositionResponseSchema, summary="Get Position"
)
@api_controller
def get_position(
    request: HttpRequest, position_id: str, tenant: Any = None, user: Any = None
):
    """
...
def list_pnl_snapshots(
    ...
    return PnLListResponseSchema(
        snapshots=snapshots, total_count=total_count, has_next=has_next
    )


@router.get(
    "/pnl/{snapshot_id}", response=PnLSnapshotResponseSchema, summary="Get P&L Snapshot"
)
@api_controller
def get_pnl_snapshot(
    request: HttpRequest, snapshot_id: str, tenant: Any = None, user: Any = None
):
    """
...
@router.post(
    "/pnl/snapshot", response=PnLSnapshotResponseSchema, summary="Create P&L Snapshot"
)
@api_controller
def create_pnl_snapshot(request: HttpRequest, tenant: Any = None, user: Any = None):
    """
