...
class TenantAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        (_("Trading Configuration"), {"fields": ("default_currency", "timezone")}),
        (
        ...
        (_("Status"), {"fields": ("is_active", "is_suspended", "suspension_reason")}),
...
class TenantUserAdmin(admin.ModelAdmin):
    ...
    readonly_fields = ["id", "created_at", "updated_at", "joined_at", "invited_by"]
    ordering = ["-joined_at"]

    fieldsets = (
        (_("Basic Information"), {"fields": ("tenant", "user", "role")}),
        (
            _("Permissions"),
            {
                "fields": (
        ...
        (
            _("Invitation"),
            {"fields": ("invited_by", "joined_at"), "classes": ("collapse",)},
        ),
        (
            _("Settings"),
            {"fields": ("notification_preferences",), "classes": ("collapse",)},
        ),
        (
            _("Timestamps"),
            {"fields": ("created_at", "updated_at"), "classes": ("collapse",)},
    ...
    def permissions_display(self, obj):
        ...
        if obj.can_trade:
            badges.append(
                '<span style="background: green; color: white; padding: 2px 6px; border-radius: 3px; font-size: 10px;">Trade</span>'
            )
        if obj.can_manage_users:
            badges.append(
                '<span style="background: blue; color: white; padding: 2px 6px; border-radius: 3px; font-size: 10px;">Users</span>'
            )
        if obj.can_manage_strategies:
            badges.append(
                '<span style="background: purple; color: white; padding: 2px 6px; border-radius: 3px; font-size: 10px;">Strategies</span>'
            )
        if obj.can_manage_risk:
            badges.append(
                '<span style="background: orange; color: white; padding: 2px 6px; border-radius: 3px; font-size: 10px;">Risk</span>'
            )
    ...
    def get_queryset(self, request):
        ...
        return (
            super().get_queryset(request).select_related("user", "tenant", "invited_by")
        )
...
class TenantInvitationAdmin(admin.ModelAdmin):
    ...
    list_filter = ["role", "status", "tenant", "created_at", "expires_at"]
    ...
    fieldsets = (
        (_("Basic Information"), {"fields": ("tenant", "email", "role", "invited_by")}),
        (_("Invitation Details"), {"fields": ("token", "expires_at", "status")}),
        (
            _("Response"),
            {"fields": ("responded_at", "response_notes"), "classes": ("collapse",)},
        ),
        (
            _("Email Tracking"),
            {"fields": ("sent_at", "opened_at"), "classes": ("collapse",)},
        ),
        (
            _("Timestamps"),
            {"fields": ("created_at", "updated_at"), "classes": ("collapse",)},
