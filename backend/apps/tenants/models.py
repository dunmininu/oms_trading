...
class Tenant(BaseModel):
    ...
    subdomain = models.CharField(
        ...
        unique=True,
        db_index=True,
        help_text="Subdomain for tenant-specific URLs",
    )
    ...
    default_currency = models.CharField(
        ...
        validators=[
            RegexValidator(
                regex=r"^[A-Z]{3}$", message="Currency code must be 3 uppercase letters"
            )
        ],
    )
    ...
    def can_use_system(self):
        ...
        return (
            ...
            and (self.is_trial_active or self.is_subscription_active)
        )
...
class TenantUser(BaseModel):
    ...
    tenant = models.ForeignKey(
        Tenant, on_delete=models.CASCADE, related_name="tenant_users", db_index=True
    )
...
class TenantInvitation(BaseModel):
    ...
    tenant = models.ForeignKey(
        Tenant, on_delete=models.CASCADE, related_name="invitations", db_index=True
    )
    email = models.EmailField(db_index=True)
    role = models.CharField(
        max_length=20, choices=TenantUser.ROLE_CHOICES, default="TRADER"
    )
    invited_by = models.ForeignKey(
        "core.User", on_delete=models.CASCADE, related_name="sent_invitations"
    )
    ...
    def accept(self, user):
        ...
        TenantUser.objects.create(
            tenant=self.tenant, user=user, role=self.role, invited_by=self.invited_by
        )
