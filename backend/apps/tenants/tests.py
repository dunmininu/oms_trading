...
class TenantUserModelTest(TestCase):
    ...
    def test_tenant_user_creation(self):
        ...
        tenant_user = TenantUser.objects.create(
            tenant=self.tenant, user=self.user, role="TRADER"
        )
    ...
    def test_tenant_user_unique_constraint(self):
        ...
        TenantUser.objects.create(tenant=self.tenant, user=self.user, role="TRADER")

        # Try to create duplicate
        with self.assertRaises(IntegrityError):
            TenantUser.objects.create(tenant=self.tenant, user=self.user, role="ADMIN")
    ...
    def test_tenant_user_permissions_by_role(self):
        ...
        trader_user = TenantUser.objects.create(
            ...
            user=User.objects.create_user(
                email="trader@example.com", password="testpass123"
            ),
            role="TRADER",
        )
...
class TenantInvitationModelTest(TestCase):
    ...
    def test_invitation_accept(self):
        ...
        accepting_user = User.objects.create_user(
            email="invite@example.com", password="testpass123"
        )
        ...
        tenant_user = TenantUser.objects.get(tenant=self.tenant, user=accepting_user)
        self.assertEqual(tenant_user.role, "TRADER")
...
class TenantIntegrationTest(TestCase):
    ...
    def test_tenant_user_limit_enforcement(self):
        ...
        for i in range(3):
            ...
            TenantUser.objects.create(tenant=self.tenant, user=user, role="TRADER")
        ...
        with self.assertRaises(IntegrityError):
            TenantUser.objects.create(
                tenant=self.tenant, user=extra_user, role="TRADER"
            )
    ...
    def test_tenant_cascade_deletion(self):
        ...
        TenantUser.objects.create(tenant=self.tenant, user=user1, role="TRADER")
        TenantUser.objects.create(tenant=self.tenant, user=user2, role="ADMIN")
