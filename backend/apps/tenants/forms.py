...
class TenantForm(forms.ModelForm):
    ...
    class Meta:
        ...
        widgets = {
            ...
            "subscription_ends_at": forms.DateTimeInput(
    ...
    def clean_name(self):
        ...
        if (
            Tenant.objects.filter(name=name)
            .exclude(pk=instance.pk if instance else None)
            .exists()
        ):
...
class TenantUserForm(forms.ModelForm):
    ...
    def clean(self):
        ...
        if tenant and user:
            ...
            existing = TenantUser.objects.filter(tenant=tenant, user=user).exclude(
            ...
            # Check tenant limits
            if (not instance or instance.tenant != tenant) and (
                tenant.tenant_users.filter(is_active=True).count() >= tenant.max_users
            ):
                raise forms.ValidationError(_("Tenant has reached maximum user limit."))
...
class TenantInvitationResponseForm(forms.Form):
    ...
    action = forms.ChoiceField(
        choices=[("accept", "Accept"), ("decline", "Decline")], widget=forms.RadioSelect
    )
...
class TenantSearchForm(forms.Form):
    ...
    search = forms.CharField(
        ...
        widget=forms.TextInput(
            attrs={"placeholder": _("Search tenants..."), "class": "form-control"}
        ),
    )

    business_type = forms.ChoiceField(
        required=False,
        choices=[("", _("All Business Types"))]
        + Tenant._meta.get_field("business_type").choices,
    ...
    subscription_plan = forms.ChoiceField(
        required=False,
        choices=[("", _("All Plans"))]
        + Tenant._meta.get_field("subscription_plan").choices,
        widget=forms.Select(attrs={"class": "form-control"}),
    )
...
class TenantUserSearchForm(forms.Form):
    ...
    search = forms.CharField(
        ...
        max_length=100,
        widget=forms.TextInput(
            attrs={"placeholder": _("Search users..."), "class": "form-control"}
        ),
    )
