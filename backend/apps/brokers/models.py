...
>>>>>>> origin/main
    ...
    broker_type = models.CharField(
        max_length=50, choices=BROKER_TYPE_CHOICES, db_index=True
    )
...
class BrokerConnection(BaseModel):
    ...
    status = models.CharField(
        max_length=20, choices=STATUS_CHOICES, default="DISCONNECTED", db_index=True
    )
    ...
    def get_connection_settings(self):
        ...
        settings = {
            ...
            "use_ssl": (
                self.use_ssl_override
                if self.use_ssl_override is not None
                else self.broker.use_ssl
            ),
...
class BrokerAccount(BaseModel):
    ...
    account_type = models.CharField(
        max_length=20, choices=ACCOUNT_TYPE_CHOICES, db_index=True
    )
    status = models.CharField(
