...
from .models import Broker, BrokerAccount, BrokerConnection, BrokerConnectionLog
...
class BrokerAdmin(admin.ModelAdmin):
    ...
    list_filter = ["broker_type", "is_active", "is_testing", "created_at"]
    ...
    fieldsets = (
        ...
        (_("Connection Settings"), {"fields": ("host", "port", "use_ssl")}),
        (
            _("API Configuration"),
            {"fields": ("api_version", "timeout_seconds", "max_retries")},
        ),
        (_("Status"), {"fields": ("is_active", "is_testing")}),
        (
...
class BrokerConnectionAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        (_("Status and Health"), {"fields": ("status", "connection_errors")}),
        (
            _("Trading Hours"),
            {
        ...
        (
            _("Risk Management"),
            {"fields": ("max_order_value", "max_daily_loss"), "classes": ("collapse",)},
        ),
        (
            _("Settings"),
...
class BrokerAccountAdmin(admin.ModelAdmin):
        ...
        (
            _("Trading Permissions"),
            {
                "fields": (
                    ...
                    "can_trade_forex",
                    "can_trade_bonds",
                    "can_trade_mutual_funds",
                )
            },
        ),
        (_("Risk Profile"), {"fields": ("risk_tolerance", "max_leverage")}),
        (
            _("Account Limits"),
            {
    ...
    def broker_connection_name(self, obj):
        ...
        if obj.broker_connection:
            url = reverse(
                "admin:brokers_brokerconnection_change", args=[obj.broker_connection.id]
            )
    ...
    def permissions_display(self, obj):
        ...
        if obj.can_trade_stocks:
            badges.append(
                '<span style="background: green; color: white; padding: 2px 6px; border-radius: 3px; font-size: 10px;">Stocks</span>'
            )
        if obj.can_trade_options:
            badges.append(
                '<span style="background: blue; color: white; padding: 2px 6px; border-radius: 3px; font-size: 10px;">Options</span>'
            )
        if obj.can_trade_futures:
            badges.append(
                '<span style="background: purple; color: white; padding: 2px 6px; border-radius: 3px; font-size: 10px;">Futures</span>'
            )
        if obj.can_trade_forex:
            badges.append(
                '<span style="background: orange; color: white; padding: 2px 6px; border-radius: 3px; font-size: 10px;">Forex</span>'
            )
...
class BrokerConnectionLogAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        (
            _("Request Details"),
            {"fields": ("request_id", "response_time_ms"), "classes": ("collapse",)},
        ),
        (
            _("Error Details"),
            {
    ...
    def response_time_display(self, obj):
        ...
        return format_html(
            '<span style="color: {};">{}ms</span>', color, obj.response_time_ms
        )
