...
from apps.oms.models import Order, Instrument
...
class Command(BaseCommand):
    ...
    async def run_major_test(self):
        self.stdout.write(self.style.MIGRATE_HEADING("=== MAJOR END-TO-END VERIFICATION (PHASE 1 & 2) ==="))
        ...
        if ticks and ticks[0]['p'] == 50000.0:
            self.stdout.write(self.style.SUCCESS("Cache verified: Sub-millisecond tick access ready."))
        ...
        self.stdout.write(f"Risk Assessment (Test Win Prob {test_win_prob}): {risk_pct:.2f}% of account suggested.")
        ...
        if risk_result['allowed']:
            ...
            for _ in range(5):
                ...
                if order.state == 'FILLED':
                    self.stdout.write(self.style.SUCCESS(f"Order {order.client_order_id} FILLED via Mock IB."))
                    break
        ...
        deriv_conn, _ = BrokerConnection.objects.get_or_create(
            tenant=tenant, broker=deriv_broker, name='Test Deriv',
            defaults={'api_key': 'mock_token', 'status': 'CONNECTED'}
        )
        deriv_acc, _ = BrokerAccount.objects.get_or_create(
            tenant=tenant, broker_connection=deriv_conn, account_number='CR12345',
            defaults={'status': 'ACTIVE'}
        )

        self.stdout.write("Placing Mocked Deriv Trade for VIX100...")
        deriv_order = Order.objects.create(
            tenant=tenant, user=user, broker_account=deriv_acc, instrument=vix100,
            side='BUY', quantity=Decimal('10.0'), order_type='MARKET', state='FILLED',
            client_order_id=f"DERIV-{uuid.uuid4().hex[:8]}", filled_quantity=Decimal('10.0'),
            filled_at=timezone.now()
        )
        self.stdout.write(self.style.SUCCESS(f"Deriv Order {deriv_order.client_order_id} simulated FILLED."))

        from apps.oms.models import Position
        pos = Position.objects.filter(instrument=btcusd).first()
        if pos:
            self.stdout.write(self.style.SUCCESS(f"Final Integrated Position: {pos.quantity} BTC"))
