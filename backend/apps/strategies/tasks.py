...
def scan_instrument_setup(tenant_id, instrument_id):
    ...
    try:
        ...
        grading_result = GradingService.grade_setup(instrument, '15_MINUTE', tenant=tenant)

        if grading_result['grade'] in ['A+', 'B']:
            logger.info(f"SIGNAL FOUND: {tenant.name} - {instrument.symbol} - Grade {grading_result['grade']}")
            # In production, this would trigger trade execution if auto-start is enabled

    except Exception as e:
        logger.error(f"Error scanning instrument {instrument_id} for tenant {tenant_id}: {e}")
=======
import logging
